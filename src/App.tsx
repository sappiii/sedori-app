...
import {
  ...
  AlertCircle, Zap, ScanLine,
} from 'lucide-react'
import { Product, Marketplace, SortKey, Tab } from './types'
import { ProductCard } from './components/ProductCard'
import { searchProducts, getSavedProducts, saveProduct, deleteProduct, autoScan } from './api/client'
...
const SCAN_CATEGORIES = [
  { key: '', label: '全て' },
  { key: 'ゲーム機', label: 'ゲーム機' },
  { key: 'Apple製品', label: 'Apple製品' },
  { key: '家電', label: '家電' },
  { key: 'LEGO・おもちゃ', label: 'LEGO・おもちゃ' },
  { key: 'トレカ', label: 'トレカ' },
  { key: 'カメラ', label: 'カメラ' },
  { key: 'スポーツ', label: 'スポーツ' },
  { key: '美容健康', label: '美容健康' },
...
export default function App() {
  ...
  const [scanMinProfit, setScanMinProfit] = useState('1000')
  const [scanMinRate, setScanMinRate] = useState('10')
  const [scanCategory, setScanCategory] = useState('')
  const [scanProgress, setScanProgress] = useState<{ current: number; total: number } | null>(null)
  const [scanResults, setScanResults] = useState<Product[]>([])
  const [scanStats, setScanStats] = useState<{ scannedCount: number; hitCount: number } | null>(null)
  const [scanSortKey, setScanSortKey] = useState<SortKey>('profit')
  ...
  const handleAutoScan = async () => {
    const minP = parseInt(scanMinProfit) || 1000
    const minR = parseInt(scanMinRate) || 10

    setLoading(true)
    setError('')
    setScanResults([])
    setScanStats(null)
    // Simulate progress: we don't have real progress from a single API call,
    // so show an indeterminate spinner with scanned count from response
    setScanProgress({ current: 0, total: 65 })

    try {
      const data = await autoScan({
        minProfit: minP,
        minProfitRate: minR,
        feeRate,
        categories: scanCategory || undefined,
      })
      setScanResults(data.results)
      setScanStats({ scannedCount: data.scannedCount, hitCount: data.hitCount })
  ...
  const handleToggleSave = async (product: Product) => {
    if (saved.some(s => s.id === product.id)) {
      ...
      setScanResults(prev => prev.map(p => p.id === product.id ? { ...p, saved: false } : p))
    } else {
      ...
      setScanResults(prev => prev.map(p => p.id === product.id ? { ...p, saved: true } : p))
    }
  }

  // Source for current results tab
  const activeResults = appMode === 'scan' ? scanResults : results
  const activeHasSearched = appMode === 'scan' ? scanResults.length > 0 || loading : hasSearched

  const filtered = useMemo(() => {
    const source = tab === 'saved' ? saved : activeResults
    const minP = appMode === 'manual' ? (parseInt(minProfit) || 0) : 0
    const minR = appMode === 'manual' ? (parseInt(minRate) || 0) : 0
    const filteredItems = source.filter(p =>
      p.profit >= minP && p.profitRate >= minR &&
      (appMode === 'manual'
        ? (selectedMP.length === 0 || selectedMP.includes(p.marketplace ?? p.sourceMarketplace ?? 'amazon'))
        : true)
    )
    const sk = appMode === 'scan' ? scanSortKey : sortKey
    return [...filteredItems].sort((a, b) => {
      if (sk === 'profit') return b.profit - a.profit
      if (sk === 'profitRate') return b.profitRate - a.profitRate
      return new Date(b.savedAt).getTime() - new Date(a.savedAt).getTime()
    })
  }, [tab, activeResults, saved, minProfit, minRate, selectedMP, sortKey, scanSortKey, appMode])
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh', maxWidth: 500, margin: '0 auto' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {/* App mode toggle */}
            {(['scan', 'manual'] as AppMode[]).map(m => (
              <button
                key={m}
                onClick={() => setAppMode(m)}
                className={appMode === m ? 'tab-active' : 'tab-inactive'}
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
              {SCAN_CATEGORIES.map(cat => {
                const active = scanCategory === cat.key
                return (
                  <button
                    key={cat.key}
                    onClick={() => setScanCategory(cat.key)}
                    style={{
                      padding: '5px 12px', borderRadius: 20, fontSize: 11, fontWeight: 700,
                      cursor: 'pointer', transition: 'all 0.2s',
                      background: active ? 'rgba(14,165,233,0.2)' : 'rgba(56,189,248,0.04)',
                      border: `1px solid ${active ? 'rgba(14,165,233,0.6)' : 'rgba(56,189,248,0.1)'}`,
            ...
            {/* Profit filters + scan button */}
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <span style={{ fontSize: 11, color: 'var(--text-3)' }}>利益</span>
                <input
                  className="input-field"
                  style={{ width: 72, padding: '8px 8px', fontSize: 12, textAlign: 'center' }}
                  placeholder="1000"
                  type="number"
                  value={scanMinProfit}
                  onChange={e => setScanMinProfit(e.target.value)}
                />
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                <input
                  className="input-field"
                  style={{ width: 56, padding: '8px 8px', fontSize: 12, textAlign: 'center' }}
                  placeholder="10"
                  type="number"
                  value={scanMinRate}
                  onChange={e => setScanMinRate(e.target.value)}
                />
              ...
              <button
                ...
                disabled={loading}
                style={{ marginLeft: 'auto', padding: '8px 14px', fontSize: 12, display: 'flex', alignItems: 'center', gap: 6, whiteSpace: 'nowrap' }}
              >
                {loading ? <RefreshCw size={13} className="spin" /> : <ScanLine size={13} />}
                {loading ? 'スキャン中' : '🔍 自動スキャン開始'}
              </button>
            </div>
            ...
            {loading && scanProgress && (
              <div style={{
                ...
                fontSize: 12, color: '#38bdf8', display: 'flex', alignItems: 'center', gap: 8,
              }}>
                <RefreshCw size={12} className="spin" />
                <span>スキャン中... {scanProgress.current}/{scanProgress.total}件の商品を確認中</span>
              </div>
            )}
            ...
            {scanStats && !loading && (
              <div style={{
                ...
                fontSize: 12, color: '#34d399', display: 'flex', alignItems: 'center', gap: 8,
              }}>
                <TrendingUp size={12} />
                <span>{scanStats.scannedCount}件スキャン → {scanStats.hitCount}件ヒット</span>
              </div>
            )}

            {/* Sort row for scan results */}
            {(scanResults.length > 0 || tab === 'saved') && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                <div style={{ marginLeft: 'auto', display: 'flex', gap: 4, alignItems: 'center' }}>
                  {([
                    ...
                    <button
                      key={s.key}
                      onClick={() => setScanSortKey(s.key)}
                      className={scanSortKey === s.key ? 'tab-active' : 'tab-inactive'}
      ...
      <div style={{ display: 'flex', gap: 0, borderBottom: '1px solid rgba(56,189,248,0.1)', background: 'rgba(6,12,26,0.7)' }}>
        {([
          { key: 'search', label: appMode === 'scan' ? 'スキャン結果' : '検索結果', count: activeResults.length },
      ...
      <div style={{ padding: '14px 14px 96px' }}>
        ...
        {backendOk === false && (
          <div style={{
            ...
            バックエンドが起動していません。<code style={{ background: 'rgba(0,0,0,0.3)', padding: '1px 6px', borderRadius: 4 }}>cd backend && npm start</code> で起動してください。
          </div>
        )}

        {/* Scan mode empty state */}
        {appMode === 'scan' && tab === 'search' && !activeHasSearched && !loading && (
          <div style={{ textAlign: 'center', padding: '48px 24px' }}>
            ...
            <p style={{ color: '#e2e8f0', fontWeight: 700, fontSize: 15, marginBottom: 6 }}>自動スキャンを開始してください</p>
            <p style={{ color: 'var(--text-3)', fontSize: 12 }}>条件を設定して「自動スキャン開始」ボタンを押してください</p>
            <p style={{ color: 'var(--text-3)', fontSize: 11, marginTop: 4 }}>65件以上の人気商品をYahoo!ショッピングで自動検索します</p>
          </div>
        )}

        {/* Manual mode empty state */}
        {appMode === 'manual' && tab === 'search' && !hasSearched && !loading && (
          <div style={{ textAlign: 'center', padding: '48px 24px' }}>
            <div style={{
              width: 64, height: 64, borderRadius: 20, margin: '0 auto 16px',
              background: 'linear-gradient(135deg, rgba(14,165,233,0.15), rgba(129,140,248,0.15))',
        ...
        {filtered.length === 0 && (activeHasSearched || tab === 'saved') && saved.length > 0 || (filtered.length === 0 && activeHasSearched && tab === 'search') ? (
      ...
      <div style={{
        ...
        {tab === 'search' && appMode === 'scan' && scanResults.length > 0 && (
          <button
            className="btn-primary"
            onClick={handleAutoScan}
            disabled={loading}
        ...
        {tab === 'search' && appMode === 'manual' && hasSearched && (
          <button
            className="btn-primary"
            onClick={handleSearch}
            disabled={loading}
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ padding: '12px', borderRadius: 10, background: 'rgba(56,189,248,0.05)', border: '1px solid rgba(56,189,248,0.1)', fontSize: 12, color: 'var(--text-2)', lineHeight: 1.6 }}>
                <p style={{ fontWeight: 600, color: 'var(--primary)', marginBottom: 6 }}>表示モード</p>
